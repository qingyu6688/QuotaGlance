...
当前工程已包含内存态快照、刷新状态和运行期偏好 JSON。窗口模式、置顶、穿透与主题偏好已使用临时文件、有效备份和恢复原因落盘；项目仍未创建数据库或迁移文件。

### 1.1 0.1.0 当前实现边界

- 动态 Rust 领域、严格 JSONL、只读常驻 App Server 会话、内存 pending request map、基础刷新状态、9 个 IPC、窗口/托盘偏好和 React UI 已实现。
...
| `locale` | 枚举 | 是 | `system` | `system`、`zh-CN`、`en` |
...
| 字段 | 类型 | 默认值 | 规则 |
|---|---|---:|---|
| `enabled` | 布尔值 | `true` | 只控制本地通知，不改变刷新策略 |
| `warningRemainingPercent` | 整数 | `20` | `0..=100` |
| `criticalRemainingPercent` | 整数 | `10` | `0..=100`，且不大于 warning |
| `notifyWhenRecovered` | 布尔值 | `true` | 从低额度或错误状态恢复时是否通知 |
...
设置开机启动时，应先完成平台调用，再提交偏好原子写入。平台调用失败则保留旧偏好，避免配置声称已启用而系统实际未启用。应用启动时可核对期望与实际状态，并以可理解错误提示差异。
