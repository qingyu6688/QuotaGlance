...
> 文档状态：安全设计初稿  
> 核对日期：2026-07-14
> 当前阶段：0.1.5 社区预览已实现部分控制并通过单元测试，尚未完成完整安全审计与跨平台实机验证
...
当前代码已实现只读方法白名单、类型化协议模型、固定参数且不经过 Shell 的进程启动、JSONL 大小限制与请求超时，并将 WebView Capability 收敛到最小范围。假 App Server 契约覆盖未知请求拒绝、错误脱敏、畸形输出、超时和异常退出；Windows 真实 POC 通过 Codex 自身登录态读取额度，未读取认证文件或凭据内容。以上仍不能替代 production sidecar 供应链、双平台运行时、签名更新和安装器的完整安全审计。
...
| 私有接口或凭据读取被错误引入 | 凭据暴露、误报额度 | 架构禁用、依赖与网络审查、无通用网络权限、负向测试 | 静态审查与网络观察 |
...
- 不扫描用户项目、主目录或浏览器数据；
- 本地偏好只写应用自己的数据目录；
- 登录时启动只在用户明确开启时创建最小自启动项，偏好保存失败时回滚系统状态；
- 通知权限只在需要本地额度提醒时申请；
- 不申请摄像头、麦克风、位置、通讯录或屏幕录制权限。

### 7.2 Tauri 权限

`widget`、`settings` 等窗口分别声明能力。只授予业务所需的事件、窗口、托盘、通知、登录时启动和 updater 权限。
...
`0.1.5` 公开预览版不捆绑或重新分发 Codex App Server。它会复用用户已经安装的 Codex：Windows 优先使用 Codex 桌面应用管理的运行副本，随后检查 `PATH`；macOS 依次检查系统级与用户级统一版 `ChatGPT.app`、两处旧版 `Codex.app`，最后回退到 `PATH` 与常见 CLI 目录；Linux 检查 `PATH` 与常见用户安装目录。候选路径必须规范化为普通文件，Unix 还必须具有可执行位；macOS 应用包或运行文件的符号链接不得逃逸允许的“应用程序”目录。启动参数固定为 `app-server`，不经过 Shell，也不会读取 `auth.json` 或 Token。所有候选仍标记为用户本机的 `External` 安装；当前仅校验路径边界和可执行属性，尚未验证 bundle identifier、OpenAI 签名身份或 Team requirement，因此 Release 必须明确标记为预览版，正式 M0 仍以来源和签名验证为门禁。
...
QuotaGlance 不提供、也不保留 `LegacyWhamProvider` 或类似备用数据源。App Server 缺失、离线、过旧或协议变化时必须进入明确错误或旧数据状态，不得自动或手动降级到直接凭据读取。

以下能力在当前版本与 1.0 范围内均明确禁止：

- 读取、解析、复制或监听 `CODEX_HOME/auth.json` 及官方默认认证文件；
- 访问系统 keyring 获取 Codex 凭据；
- 从 ChatGPT、浏览器、桌面应用或进程内存提取 Token、Cookie 或 Authorization Header；
- 由 QuotaGlance 直接调用非公开 `chatgpt.com/backend-api/wham/*` 接口；
- 通过代理、注入、重定向或中间人方式复用认证流量；
- 因 App Server 故障而要求用户粘贴凭据或上传认证文件；
- 调用重置机会兑换、购买、发信、登录、登出或其他账号写操作。

参考项目采用的凭据与私有 HTTP 路线只用于理解其可见字段和用户体验，不进入 QuotaGlance 的实现、测试 fixture 或发布包。若未来公开协议无法提供某一字段，应隐藏该可选字段并记录兼容性问题，而不是扩大凭据权限。
...
- [ ] 仓库与运行时观察均证明不存在凭据读取 Provider 和非公开 `wham` 请求；
- [ ] 依赖审计没有未接受的高危问题；
...
- 确认泄露来自 QuotaGlance、Codex App Server 还是外部环境；
