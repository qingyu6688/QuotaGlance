...
> 文档状态：安全设计初稿  
> 核对日期：2026-07-14
> 当前阶段：0.1.4 社区预览已实现部分控制并通过单元测试，尚未完成完整安全审计与跨平台实机验证
...
`0.1.4` 公开预览版不捆绑或重新分发 Codex App Server。它会复用用户已经安装的 Codex：Windows 优先使用 Codex 桌面应用管理的运行副本，随后检查 `PATH`；macOS 依次检查系统级与用户级统一版 `ChatGPT.app`、两处旧版 `Codex.app`，最后回退到 `PATH` 与常见 CLI 目录；Linux 检查 `PATH` 与常见用户安装目录。候选路径必须规范化为普通文件，Unix 还必须具有可执行位；macOS 应用包或运行文件的符号链接不得逃逸允许的“应用程序”目录。启动参数固定为 `app-server`，不经过 Shell，也不会读取 `auth.json` 或 Token。所有候选仍标记为用户本机的 `External` 安装；当前仅校验路径边界和可执行属性，尚未验证 bundle identifier、OpenAI 签名身份或 Team requirement，因此 Release 必须明确标记为预览版，正式 M0 仍以来源和签名验证为门禁。
