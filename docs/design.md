...
- 窗口模式、置顶、穿透和主题偏好已原子落盘并支持备份恢复；语言、窗口边界、更新器和 sidecar 生产分发仍是后续实施内容。

## 2. 系统目标与边界

QuotaGlance 是一个面向 Windows 和 macOS 的本地桌面额度助手。它通过浮球、展开卡片和系统托盘，近实时展示当前 Codex 额度桶、额度窗口、剩余比例、重置时间和数据状态。
...
- 不默认启用 `LegacyWhamProvider` 或其他非公开接口。
...
| `LegacyWhamProvider` | 用户明确启用后的隔离兼容路径 | 默认关闭，不作为可用性验收前提 |
