...
`0.1.6` 视觉优化使用以下卡片、设置与浮球概念：

![QuotaGlance 卡片、设置与浮球概念](design-assets/quotaglance-ui-concept-v2.png)

概念图用于确定色彩、排版、组件几何和交互方向。`0.1.6` 继续以周额度作为悬浮球和展开卡片的主读数，并在 App Server 实际返回时补充套餐、短周期/5 小时额度、可用重置机会与到期时间；缺失字段不占位，也不伪造成 `0`。实际界面文字、按钮、进度条和图标必须使用 React、HTML、CSS 与 SVG 实现，不把概念图作为应用背景截图。
...
- 视觉关键词：安静、准确、可信、轻量。
- 主界面是一块完整圆角玻璃额度面板，信息详情使用轻量内嵌行，不堆叠彼此竞争的卡片。
- 信息按“身份与操作 → 周额度主读数 → 短周期/重置机会整行详情 → 新鲜度”纵向排列；短周期和重置机会不再压缩为两列小字。
- 健康状态以冷蓝色表达；提醒和危险状态只替换语义色，不改变整体布局。
- 浮球采用类似桌面加速球的完整球形轮廓，以多层玻璃高光、内侧暗部和圆形边缘形成三维体积；球内按“周额度 → 百分比 → 重置日期 → 状态”四层排版。进度环与数字精确表达周额度，水面在 44–64px 安全区间内随额度变化，并使用低幅惯性倾斜、水平位移和回弹，保证重置日期与状态始终位于液体区域内。
...
| 组件 | 目标尺寸 | 说明 |
|---|---:|---|
| `QuotaCard` | 320 × 320px | 默认展开状态；详情保持在固定桌面窗口内，缺失字段不占位 |
| `QuotaOrb` | 136 × 136px 窗口，128 × 128px 可见球体 | 正圆玻璃浮球，显示标题、百分比、重置日期、状态、动态水面和进度环 |
| `IconButton` | 32 × 32px | 统一图标、内收焦点环、悬停和按下状态 |
| `SettingsPanel` | 308 × 308px 可用区域 | 覆盖卡片主体的完整玻璃对话框；必要时仅面板内部垂直滚动 |
| `ThemeControl` | 七列单行网格 | 在设置面板内切换七套主题；使用圆形真实色样、文字、选中边框和勾选图标共同表达状态 |
...
- 设置按钮打开完整玻璃设置面板；面板包含显示模式、七主题、置顶、登录时启动和鼠标穿透，并沿用相同令牌和排版。
- 设置面板使用对话框语义并约束 Tab / Shift+Tab 焦点；主题单选项支持方向键、Home 和 End，Escape 关闭后把焦点归还设置按钮。
...
- `ok`：按当前主窗口剩余比例使用健康、提醒或危险语义色；有周额度时浮球数字与圆环精确同步，水面在安全视觉区间内联动，短周期和重置机会作为卡片补充信息。
...
截至 2026-07-14，已使用浏览器开发预览与 4 个 Vitest 文件、26 项测试检查正式 React 组件。该记录只证明界面组件和数据投影在当前开发环境中符合基线，不替代 Windows WebView2、macOS WKWebView、Linux WebKitGTK、多屏或系统缩放实测。

| 对照项 | 实现结果 | 结论 |
|---|---|---|
| 信息层级 | 使用“身份与操作 → 周额度主读数 → 短周期/重置机会整行详情 → 新鲜度”；操作反馈独立显示，浮球保持周额度四层信息 | 通过 |
| 组件几何 | 卡片实测 `320 × 320px`；浮球窗口 `136 × 136px`，可见球体 `128 × 128px` 且圆角为 `50%` | 通过 |
| 排版 | 卡片周额度主读数 60px；短周期和重置机会使用 38px 高整行详情；浮球四层信息均位于球壳内 | 通过 |
| 色彩与状态 | 正常、提醒、危险和旧数据使用独立语义色；错误状态不显示假 `0%` | 通过 |
| 桌面画布 | `1200 × 800` 下无横纵向溢出，卡片内容无滚动 | 通过 |
| 窄视口 | `400 × 720` 下卡片居中，无横纵向溢出 | 通过 |
| 键盘操作 | 卡片与浮球支持 Enter/Space；浮球支持菜单键；设置约束焦点、支持主题方向键和 Escape 关闭归还焦点 | 通过 |
| 运行状态 | 状态切换、置顶反馈、卡片/浮球切换和动态水面均有可见结果，控制台无警告或错误 | 通过 |
| 主题设置 | 七套主题以单行圆形色样呈现；卡片边框、球壳、水体、水线、气泡和状态色同步切换，暖色主题不混入默认蓝色 | 通过 |
...
- 概念图同时展示卡片、设置与浮球三种状态；实际桌面窗口一次只显示当前模式，切换由本地偏好与 Tauri 窗口状态共同控制。
- 当前悬浮球仍以周额度驱动水位，展开卡片只在 App Server 实际返回时补充短周期/5 小时额度与重置机会。
...
- 概念图用于表达层级，最终实现额外保留置顶、登录时启动、鼠标穿透和数据新鲜度等真实桌面状态。
