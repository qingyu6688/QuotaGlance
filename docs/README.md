...
当前工程版本为 `0.1.0`，项目已从纯文档阶段进入 **M0/M1/M2 实施阶段**：前端 14 项测试和 Rust 45 项测试通过，Windows Release 已通过当前 Codex 桌面运行副本读取真实账号额度。以上仍只是 Windows 10 开发机结果，不等于完成固定版本、完整认证矩阵、Windows 11、macOS、签名、公证或正式稳定版验收。
...
| 自动化测试 | 前端 14 项、Rust 45 项测试通过 | 当前开发机通过；含 9 项假服务跨进程协议契约、Windows 受管运行时和跨平台 `PATH` 定位测试 |
