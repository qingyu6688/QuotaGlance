...
当前工程版本为 `0.1.6`，项目已进入 **M0/M1/M2/M3 实施阶段**：前端 26 项测试和 Rust 45 项测试通过，Windows Release 模式构建与浮球原生拖拽/右键菜单烟测通过，并完成卡片、设置、七主题和动态水面浮球的本轮界面优化。以上仍主要是 Windows 10 开发机证据，不等于完成固定 bundled sidecar、完整认证矩阵、Windows 11、macOS/Linux 实机、签名、公证或正式稳定版验收。
...
| [环境说明](environment.md) | 工具链、端口、环境变量和平台验证边界 | 已按 `0.1.6` 更新 |
