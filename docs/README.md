...
当前工程版本为 `0.1.5`，项目已进入 **M0/M1/M2/M3 实施阶段**：前端 20 项测试和 Rust 45 项测试通过，Windows Release 模式构建与浮球原生拖拽烟测通过，并补齐套餐、短周期、周额度、重置机会、macOS 统一 ChatGPT 应用发现和登录时启动。以上仍主要是 Windows 10 开发机证据，不等于完成固定 bundled sidecar、完整认证矩阵、Windows 11、macOS/Linux 实机、签名、公证或正式稳定版验收。
...
| 前端 | 额度卡片、可拖拽球形悬浮球、两项右键菜单、设置面板、模拟状态与 Tauri API 适配已落地 | 工程验证通过，产品验收未完成 |
| 协议与领域层 | 动态额度模型、JSONL 边界、只读 App Server 常驻会话与 pending request map 已落地 | 合成契约与当前 Windows 登录账号 POC 已建立；固定兼容版本和合法 bundled sidecar 待验证 |
| 自动化测试 | 前端 20 项、Rust 45 项测试通过 | 当前开发机通过；含额度投影、浮球拖拽回归、9 项假服务跨进程协议契约、Windows 受管运行时和跨平台运行时定位测试 |
| 构建与运行 | 前端构建、Tauri debug `--no-bundle` 构建、原生浮球拖拽和进程回收烟测通过 | 仅当前 Windows 开发机证据 |
| 界面验证 | 浏览器模式完成视觉和交互 QA | 使用模拟数据，不代表真实账号联调 |
| sidecar | release 尚无 bundled sidecar | 正式数据链路不可发布 |
| 平台与发布 | GitHub Actions 已构建 Windows、macOS、Linux 的 Latest 社区预览软件和 SHA-256 清单；签名、公证与全平台实机仍未验证 | 禁止标记为 1.0 正式稳定版 |
...
| [未完成问题分析](issues-analysis.md) | 未完成项分级、根因、方案、状态和解除条件 | 2026-07-14 审计基线 |
| [环境说明](environment.md) | 工具链、端口、环境变量和平台验证边界 | 已按 `0.1.5` 更新 |
...
| [部署与发布](deploy.md) | 构建、签名、公证、更新和回滚 | 跨平台 Latest 社区预览已建立；1.0 稳定版仍在规划 |
