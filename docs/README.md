...
当前工程版本为 `0.1.1`，项目已从纯文档阶段进入 **M0/M1/M2 实施阶段**：前端 15 项测试和 Rust 45 项测试通过，Windows Release 已通过当前 Codex 桌面运行副本读取真实账号额度，浮球原生拖拽烟测通过。以上仍只是 Windows 10 开发机结果，不等于完成固定版本、完整认证矩阵、Windows 11、macOS、签名、公证或正式稳定版验收。
...
| 自动化测试 | 前端 15 项、Rust 45 项测试通过 | 当前开发机通过；含浮球拖拽回归、9 项假服务跨进程协议契约、Windows 受管运行时和跨平台 `PATH` 定位测试 |
...
| 平台与发布 | GitHub Actions 已构建 Windows、macOS、Linux 跨平台 prerelease；签名、公证与全平台实机仍未验证 | 禁止标记为正式稳定版 |
...
| [环境说明](environment.md) | 工具链、端口、环境变量和平台验证边界 | 已按 `0.1.1` 更新 |
...
| [部署与发布](deploy.md) | 构建、签名、公证、更新和回滚 | 跨平台 prerelease 已建立；稳定版仍在规划 |
