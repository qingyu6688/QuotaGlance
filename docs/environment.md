...
- Rust 44 项测试通过，其中包含 9 项假 App Server 跨进程契约和受管 Codex 运行时定位测试；
...
当前记录：44 项 Rust 测试通过。`test-support` 特性只用于构建假 App Server 和跨进程契约，默认 Tauri 构建不会携带该测试程序。
...
未设置时，Windows 构建会先查找 Codex 桌面应用在 `%LOCALAPPDATA%\OpenAI\Codex\bin\<managed-id>` 下管理的运行副本；debug 构建再尝试从 `PATH` 查找 `codex`。当前 release 尚无 bundled sidecar，正式发布前仍须完成固定版本、来源、哈希、许可和签名方案。
...
在上述验证完成前，不得在 README、发布页或安装包文案中宣称 macOS 已支持。
