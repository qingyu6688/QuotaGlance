...
- 尚未使用真实 Codex 账号验证 App Server 与 sidecar 数据链路；
- 尚未执行 macOS、签名、安装器、升级和卸载验证；
...
| `cargo test --all-targets --all-features` | 通过 | 44 个 Rust 测试通过：35 个单元测试、9 个假 App Server 跨进程契约 |
...
| 窄视口 | 待本轮浏览器终检 | 验证卡片与 136 × 136 px 浮球模式均可居中显示且无溢出 |
...
- 未执行 macOS Intel 或 Apple Silicon 构建与实机测试。
- 已生成 Windows x64 未签名预览安装器，但未完成 Authenticode、Windows 11 安装/升级/卸载；macOS 签名安装器和更新均未生成或验证。
- RustSec 审计已执行且未发现漏洞；17 个维护或不健全警告已登记，后续依赖升级和发布 CI 仍需持续复核。
- 未执行完整桌面 E2E、长驻稳定性、性能和多屏测试。

结论：开发基线检查通过，**禁止发布**。完成真实 App Server/sidecar、双平台、签名安装器和依赖审计验证前，不得将本报告作为正式版本验收依据。
